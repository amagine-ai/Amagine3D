...
ZIP files, restored OPFS data, attachment metadata, URLs, and filenames as
...
| Web Research        | Prompt injection and false dimensions                                               | Separate research stage and tools, no raw HTML, bounded structured summaries, HTTP(S)-only URLs, advisory-only packet, user constraints win, fail-soft behavior                                                 | Sources can still be wrong. Review cited mechanical drawings before fabrication.                                                             |
...
| OPFS                | Corruption, partial writes, stale schema, origin data loss                          | Committed JSON writes, schema validation/migration, quarantine, immutable runs, content hashes, ZIP backup/restore                                                                                              | Browser site-data clearing can erase OPFS. Export backups regularly.                                                                         |
...
| Downloads           | Path-like or control-character filename abuse                                       | Artifact schemas, OPFS storage-segment validation, safe leaf-name normalization before browser download                                                                                                         | Operating systems may still rewrite filenames. Verify extension/content before opening elsewhere.                                            |
...
| Secrets             | Key exposure through UI, logs, OPFS, Worker, or ZIP                                 | Server-only env access, browser requests use profile IDs, no provider SDK in browser packages, no secrets in project schemas                                                                                    | Local `.env.local` remains operator-managed and must not be committed. Rotate a key after suspected exposure.                                |
