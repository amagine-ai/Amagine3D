...
MODE_CAPABILITIES = [
    ...
    {
        "id": "hybrid",
        "physicalAuthority": "one BRep or watertight mesh master per part",
        "outputs": ["eligible part STEP", "part STL", "3MF", "display GLB"],
        "requirements": [
            "one representation master per part",
            "precise manufacturing features share the semantic scene",
        ],
    },
    {
...
MODELING_RECIPES = [
    {
        "id": "scaled-round-volume",
        "requires": ["Sphere", "scale"],
        "useWhen": "a smooth closed volume is controlled by independent axis extents",
    },
    {
...
def build_manifest(symbols: Iterable[str] = ()) -> dict[str, Any]:
    ...
    recipes = [
        {
            **recipe,
            "available": set(recipe["requires"]).issubset(available),
        }
