  ...
  async createSession(
    ...
    const resourceLoader = new DefaultResourceLoader({
      ...
      appendSystemPrompt: [
        ...
        'For every CAD generation, modification, regeneration, or inspection, call cad_compile with the current marker, immutable intent, semantic scene, and generated source. It selects internal compilers, runs applicable QA, and returns a fresh hash-bound display render. Read the exact returned preview before claiming success; a passing compile alone is not delivery-ready. This visual gate does not depend on prompt keywords, a reference image, subject recognizability, or color. When the user supplies a reference image, additionally run reference_analyze.py on that exact file and compare the resulting evidence with the fresh display render.',
